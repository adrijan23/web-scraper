...
function get_all_categories(){
    $reg='/.*?><a title=".*?" href="(https:\/\/crom\.ba\/product-category\/[a-z|-]*)\/"/';
    $data= file_get_contents('https://crom.ba');
    preg_match_all($reg, $data, $matches);
...
function all_pages($all_category_urls)
{
    foreach ($all_category_urls as $cat_url) {
        ...
        $reg = '/<li><a class="page-numbers" href=".*?">([0-9]*)<\/a><\/li>\n.*?<li><a class="next page-numbers"/';
...
{   
    ...
    foreach ($page_urls as $page) {
        $reg = '/<a href="(https:\/\/crom\.ba\/product\/.*?)\/" class="woocommerce-LoopProduct-link woocommerce-loop-product__link"><h2 class="woocommerce-loop-product__title">/';
...
function get_title_price_pics($all_product_urls)
{
    ...
    foreach ($all_product_urls as $product_url) {
        ...
        //GET CATEGORY
        $reg = '/href="https:\/\/crom\.ba\/product-category\/.*?">(.*?)<\/a><span class="delimiter"><i class="fa fa-angle-right"><\/i><\/span>.*?<\/nav>/';
        ...
        //GET DESCRIPTION
        $reg = '/<div class="electro-description clearfix">(.*?)<div class="product_meta">/s';
