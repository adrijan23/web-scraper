...
function get_all_categories(){
    $reg='/<a href="(https:\/\/technoshop\.ba\/proizvodi\/[a-z|-]+\/)"/';
    $data= file_get_contents('https://technoshop.ba/');
    preg_match_all($reg, $data, $matches);
...
function all_pages($all_category_urls)
{
    foreach ($all_category_urls as $cat_url) {
        ...
        $reg = '/<a data-shorcut="p .*?" href=".*?">([0-9]*)<\/a>\n\s*?\n\s*?<a.*?rel="next"/';
        ...
        $arr[] = $cat_url . "?to_page=" . $pagination;
...
{   
    ...
    foreach ($page_urls as $page) {
        $reg = '/<h2 data-product_title.*?><a href="(.*?)\/">/';
...
function get_title_price_pics($all_product_urls)
{
    ...
    foreach ($all_product_urls as $product_url) {
        ...
        //GET PICTURE
        $reg = '/<a href="(.*?)" class="fancybox".*?>/';
        preg_match($reg, $data, $matchPics);

        if (count($matchPics) > 0) {
            $pictures = 'https://technoshop.ba' . $matchPics[1];
        ...
        //GET DESCRIPTION
        $reg = '/<div.*?class="tab tab-description.*?>(.|\n)*?<\/div>/';
