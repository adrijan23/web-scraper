...
function get_all_categories(){
    $reg='/<li class="cat-item.*?"><a href="(https:\/\/www\.tronex\.ba\/kategorija\/[a-zA-Z|-]+)\/">/';
    $data= file_get_contents('https://www.tronex.ba/katalog/');
    preg_match_all($reg, $data, $matches);
...
function all_pages($all_category_urls)
{
    foreach ($all_category_urls as $cat_url) {
        ...
        $reg = '/([0-9]+)<\/a><\/li>(\n|\s)*<li><a class="next page-numbers" href=".*?">/';
...
function get_title_price_pics($all_product_urls)
{
    ...
    foreach ($all_product_urls as $product_url) {
        ...
        //GET TITLE
        $reg = '/<h1.*?class="product_title entry-title">(.*?)</';
        ...
        //GET PRICE
        $reg = '/<p class="price">(<del>.*?<ins>)?<span class="woocommerce-Price-amount amount">(.*?)&nbsp;/';
        ...
        //GET PICTURE
        $reg = '/<a class="woocommerce-main-image.*?<img.*?src="(.*?)"/';
        ...
        //GET CATEGORY
        $reg = '/<div class="products-page-cats">\n.*?>(.*?)<\/a>/';
        ...
        //GET DESCRIPTION
        $reg = '/<div class="tab-content-scroll">(.|\n)*?<\/div>/';
