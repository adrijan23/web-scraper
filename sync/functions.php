...
{
    $reg = '/<a href="(https:\/\/sync\.ba\/product-category\/[a-z|-]+)\/"/';
    $data = file_get_contents('https://sync.ba');
    preg_match_all($reg, $data, $matches);
...
function get_products($page_urls)
{
    ...
    foreach ($page_urls as $page) {
        $reg = '/<a href="(https:\/\/sync\.ba\/product\/.*?)\/" class="woocommerce-LoopProduct-link woocommerce-loop-product__link"/';
...
function get_title_price_pics($all_product_urls)
{
    ...
    foreach ($all_product_urls as $product_url) {
        ...
        //GET TITLE
        $reg = '/<h1 class="product_title entry-title">(.*?)<\/h1>/';
        ...
        //GET PRICE
        $reg = '/<p class="price"><span class="woocommerce-Price-amount amount"><bdi>(.*?)&nbsp;/';
        preg_match($reg, $data, $matchPrice);

        if (count($matchPrice) > 0) {
            $price = $matchPrice[1];
        ...
        //GET PICTURE
        $reg = '/class="woocommerce-product-gallery__image"><a href="(.*?)">/';
        ...
        //GET CATEGORY
        $reg = '/<nav class="woocommerce-breadcrumb"><a href=".*?">Početna<\/a>&nbsp;&#47;&nbsp;<a href=".*?">(.*?)</';
        ...
        //GET DESCRIPTION
        //$single_product['desc'] = 'Nedovrseno';
        $reg = '/<div class="woocommerce-Tabs-panel woocommerce-Tabs-panel--specification panel entry-content wc-tab" id="tab-specification" role="tabpanel" aria-labelledby="tab-title-specification">(.*?)<\/div>/s';
